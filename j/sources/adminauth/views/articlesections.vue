...
<ul class="nav nav-tabs" role="tablist">
	...
	<div class="tab-content">
		<div class="tab-pane fade " id="acatlist" role="tabpanel" aria-labelledby="list-tab">
			<div class="card">
				<div class="card-body">
					<h5 class="card-title">Список категорий статей</h5>
					
					<table id="articlecats" class="display table table-bordered" style="width:100%">
						<thead>
							<tr>
								<th>{{ $t('app.Heading') }}</th> 
		...
		<div class="tab-pane fade show active" id="editacat" role="tabpanel" aria-labelledby="edit-tab">
			<div class="card">
				<div class="card-body">
        ...
        methods:{
			...
			*/
			onClickEditCategory(evt) {
				...
				this.$root._get((d) => {this.onSuccessGetCategory(d);}, `/p/acategories/category.jn/?id=${this.requestedCategoryId}`, (a, b, c) => {this.onFailGetCategory(a, b, c);} );
			},
			/**
			...
			*/
			onClickConfirmRemoveCategory() {
				...
				this.$root._post(args, (data) => {this.onSuccessRemove(data);}, '/p/acategories/removecategory.jn/', (data) => {this.onFailRemove(data);})
			...
			*/
			...
			initSeotab() {
				...
				$('#editacat-tab').on('shown.bs.tab', (ev) => {
					this.setDataChanges(false);
				});
			},
			/**
