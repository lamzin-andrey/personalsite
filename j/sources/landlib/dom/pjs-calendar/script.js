...
(
	function () {
		...
		var T = window.PureJsCalendar.Locale,
		/*признак инпута, который надо сделать календарем*/
		CALENDAR = 'pjs-calendar',
		/*формат*/
		FORMAT = "d.m.Y",
		IMAGE_FOLDER = '/j/sources/landlib/dom/pjs-calendar',
		...
		/*========== Инициализация*/
		function onDomReady() {
			window.PureJsCalendar.list = [];
			var ls = $$(D, "input", CALENDAR), i, node;
			for (i = 0; i < ls.length; i++) {
				window.PureJsCalendar.append(ls[i]);
			}
		}
		if (W.attachEvent) {
...
<table class="' + SET + '">\
	...
	<tr class="pc-year">\
		<td colspan="7">\
			<img src="' + IMAGE_FOLDER + '/left.png" class="' + BTN_PREV_YEAR + '"/><span class="' + YEAR_STRING + '">2014</span><img src="' + IMAGE_FOLDER + '/right.png" class="' + BTN_NEXT_YEAR + '"/>\
		</td>\
	</tr>\
	<tr class="pc-month">\
		<td colspan="7">\
			<img src="' + IMAGE_FOLDER + '/left.png" class="' + BTN_PREV_MONTH + '"/><span class="' + MONTH_STRING + '">Сентябрь</span><img src="' + IMAGE_FOLDER + '/right.png" class="' + BTN_NEXT_MONTH + '"/>\
		</td>\
	</tr>\
	<tr class="pc-week">\
		<td>' + T.MONDAY + '</td>\
