...
Vue.prototype.$formTokenName = 'articleslist[_token]';
...
window.app = new Vue({
   ...
   */
   mounted() {
		this.resetCsrfToken($('#formToken').val());
		this.initSidebar();
        this.initSeotab();
        this.initDataTables();
		this.localizeParams();
   ...
   */
    ...
    */
    initDataTables() {
        ...
        this.dataTable =  $(id).DataTable( {
            ...
            'serverSide': true,
            'ajax': {
				url: that.$webRoot + "/articleslist.json",
				data: function ( d ) {
					d[that.$formTokenName] = that.formToken;
				}
			},
            "columns": [
				{ 
    ...
    */
	...
	/**
	 * @description Переустановить токен
	 * @param {String} sToken 
	*/
	resetCsrfToken(sToken) {
		this.formToken = sToken;
		Rest[this.$formTokenName] = this._getToken();
	}
   }//end methods
