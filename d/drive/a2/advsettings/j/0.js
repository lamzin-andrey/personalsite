...
function setListeners() {
	...
	e("bAgreeAdv").onchange = onChangeBtn;
}
...
function setLang(lng, displayLng) {
	...
	if (s) {
		...
		if (q == s && !isErr) {
      if('langEn' == lng) {
        q = '<div>You must be logged in to be able to disable ad impressions.</div>';
      } else {
        q = L['hWaitGetLink'];
      }
...
function onChangeBtn() {
	Rest._post({s: (e("bAgreeAdv").checked ? 1 : 0)}, onSuccessChangeAgreeAdv,
			br + '/drivesetadv.json',
			onFailGetDLink
	);
}

function onSuccessChangeAgreeAdv(data){
	if (this.onFailGetDLink(data)) {
		show('dBtnWrap');
		removeClass('dBtnWrap', 'hide');
		e('hWaitGetLink').innerHTML = e("bAgreeAdv").checked ? l("You have allowed advertising to be displayed on this site") : l("You have blocked advertising on this site");
	}
}
