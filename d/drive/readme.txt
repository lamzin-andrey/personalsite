...
7 Надо сделать логин по email
 ...
 Бэкенд.
 При вводе email запрашивается метод /checkmail. (начат в ветке try_force_login)
 Этот метод ищет, есть ли юзер с таким email.
 Если есть, 
  Записывает в UserTempPasswords хеш и дату, до которой он действтелен (сутки).
  отправляет письмо
  и возвращает ссылку на почтовый сервис.
  
 Этот же метод ищет, есть ли юзер с таким username как email.
 Если есть, генерит незанятый username.
 Возвращает на фронтед username.
 
 Если email не занят, авторегистрацией занимается фронтенд, используя методы register и login
 
 При переходе по ссылке запрашивается то, что сейчас называется /loginforce
 
 
8 Из всего что не a2 надо выпилить пункт Leave WeebUSB
