...
function main() {
	$wKey = fopen(__DIR__ . '/ozhegov_parsed.txt', 'r');
	$n = 0;
	while (!feof($wKey)) {
		//$key  = fgets($hKey);
		$word = fgets($wKey);
		...
		if (trim($key)) {
			...
			$sql = "INSERT INTO sts (`word`, `description`, `source`) VALUES ('{$key}', '{$word}', 'ozh')";
...
function parseLine(&$word, &$key) {
	$m = mb_convert_encoding(',', 'Windows-1251', 'UTF-8');
	$md = mb_convert_encoding(' ', 'Windows-1251', 'UTF-8');
	$w = mb_convert_encoding('-.', 'Windows-1251', 'UTF-8');
