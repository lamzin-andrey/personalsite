...
WHERE m.is_deleted = 1 LIMIT 10, 10; *</li>
		...
		FOSUserBundle в статье через composer ставится.
				<ul>
					<li><a href="http://127.0.0.1:8000/resetting/request" target="_blank">Восстановление</a></li>
					<li>Миграции, что и как, надо скопировать phone в username</li>
					...
					<li>Сделать так, чтобы адрес для восстановления пароля был /remind. 
					Маршруты копируем (куда?), чтобы не сломались при обновлении. Скорее всего в созданном config/routes/fos_users вместо строк импорта прописываем наши url </li>
					<li>Темизация страниц логина, которые не темизированы через bs</li>
	...
	<ul>
		<li >Разобраться с счётчиками freesoft
			...
			<li>01 11 2019 у них были проблемы с редактированием раздела.
				...
				Там что-то чинят, до 18 ноября не буду им нервы мотать.
			</li>
