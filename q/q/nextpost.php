	...
	*/
	public function _actionPost() {
		if (isset($_COOKIE[AUTH_COOKIE_NAME])) {
			$uid = (int)dbvalue("SELECT id FROM secure_pad_users WHERE auth_id = '{$_COOKIE[AUTH_COOKIE_NAME]}'");
			if ($uid) {
				$id = ireq('id', 'GET');
				$row = false;
				if ($id) {
					$row = dbrow("SELECT id, title, body FROM secure_pad_posts WHERE uid = {$uid} AND is_deleted = 0 AND id < {$id} ORDER BY id DESC LIMIT 1");
				}
				if ($row) {
